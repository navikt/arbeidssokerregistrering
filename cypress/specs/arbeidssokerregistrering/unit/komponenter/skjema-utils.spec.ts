...
describe("skjema-utils", () => {
  it("getAlleSporsmalSomIkkeSkalBesvares skal returnere riktig spørsmål", () => {
    ...
    const config: SkjemaConfig = new Map<Svar, string[]>([
      ...
      [
        ...
        ["skalHoppesOver2", "skalHoppesOver3"],
      ],
    ]);

    expect(
      getAlleSporsmalSomIkkeSkalBesvares(
        besvarteSporsmalIder as SporsmalId[],
        svar as SvarState,
        config
      )
    ).to.deep.equal(["skalHoppesOver1", "skalHoppesOver2", "skalHoppesOver3"]);
  });
  ...
  it("test getTekstIdForSvar", () => {
    ...
    ).to.equal("dinsituasjon-svar-aldri-hatt-jobb");
  });
});
