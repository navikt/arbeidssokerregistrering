...
import { Svar, UtdanningBestattSvar, UtdanningGodkjentSvar, UtdanningSvar } from "../../ducks/svar-utils";
...
const tilbakeIArbeid = (fellesProps: any, regType: any) => {
  return {
    ...
    elementOppsummering: (
      <OppsummeringElement key={SporsmalId.tilbakeIArbeid} sporsmalId={SporsmalId.tilbakeIArbeid}>
        <strong>
          <FormattedMessage id="sykmeldt-oppsummering-tilbake-i-jobb-fortekst" />
          &nbsp;
        </strong>
      </OppsummeringElement>
    ),
...
export const hentLenkeEndre = (sporsmalId: SporsmalId | undefined, svar: Svar | undefined, lop: number | undefined) => {
  ...
  if (SporsmalId.fremtidigSituasjon === sporsmalId) {
    return (
      <Link className="lenke" to="/inngangssporsmal">
        Endre
      </Link>
    );
  }
  const index = config ? config({}, "").findIndex((data) => data.id === sporsmalId) : -1;

  if (index >= 0) {
    return (
      <Link className="lenke" to={`/skjema-sykefravaer/${lop}/${index}`}>
        Endre
      </Link>
    );
  } else {
