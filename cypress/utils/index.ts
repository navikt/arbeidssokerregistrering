...
export const stdStegTest = (steg: Steg) => {
  ...
  it("Alle valg har korrekt tekst", () => {
    steg.valg.forEach((tekst: String, i: number) => {
  ...
  it("Alle valg kan velges", () => {
    steg.valg.forEach((_: String, i: number) => {
...
export const clickOptionThenNext = (n: number) => {
  cy.get('[class="alternativ-wrapper"]').eq(n).click();
  cy.get('[data-testid="neste"]').click();
};

export const clickOption = (n: number) => {
  cy.get('[class="alternativ-wrapper"]').eq(n).click();
};

export const clickNext = (n: number) => {
  cy.get('[data-testid="neste"]').click();
};

export const configureCypress = (option: String) => {
  ...
  switch (option) {
    case "registrering":
      ...
      cy.route(
        "GET",
        "/veilarbregistrering/api/startregistrering",
        "fixture:startregistrering/ordinaerregistrering"
      );
      cy.route("GET", "/veilarbperson/api/person/navn", "fixture:navn");
      cy.route(
        "GET",
        "/veilarbregistrering/api/sistearbeidsforhold",
        "fixture:sistearbeidsforhold"
      );
      cy.route(
        "GET",
        "/pam-janzz/rest/kryssklassifiserMedKonsept?kodeForOversetting=2419114",
        "fixture:kryssklassifiserMedKonsept"
      );
      cy.route(
        "GET",
        "/pam-janzz/rest/typeahead/yrke-med-styrk08?q=Klovn",
        "fixture:tidligere-yrke"
      );
      cy.route("POST", "/veilarbregistrering/api/startregistrering", {}).as(
        "startRegistrering"
      );
      break;
    case "registrering-seksavtolv":
      ...
      cy.route(
        "GET",
        "/veilarbregistrering/api/startregistrering",
        "fixture:startregistrering/ordinaerregistrering-seksavtolv"
      );
      cy.route("GET", "/veilarbperson/api/person/navn", "fixture:navn");
      cy.route(
        "GET",
    ...
    case "registrering-sykefravaer":
      ...
      cy.route(
        "POST",
        "/veilarbregistrering/api/startregistrersykmeldt",
        {}
      ).as("startregistrersykemeldt");
      cy.fixture("startregistrering/registrering-sykefravaer").then(
        (fixture) => {
    ...
    case "reaktivering":
      ...
      cy.route(
        "GET",
        "/veilarbregistrering/api/startregistrering",
        "fixture:startregistrering/reaktivering"
      );
      cy.route("GET", "/api/auth", "fixture:auth");
      cy.route("GET", featureToggles, "fixture:/feature-toggle/nedetid-false");
      cy.route("GET", "/veilarbstepup/status", "fixture:status");
    ...
    case "sperret":
      ...
      cy.route(
        "GET",
        "/veilarbregistrering/api/startregistrering",
        "fixture:startregistrering/sperret"
      );
      cy.route("GET", "/api/auth", "fixture:auth");
      cy.route("GET", featureToggles, "fixture:/feature-toggle/nedetid-false");
      cy.route("GET", "/veilarbstepup/status", "fixture:status");
    ...
    case "allerederegistrert":
      ...
      cy.route(
        "GET",
        "/veilarbregistrering/api/startregistrering",
        "fixture:startregistrering/allerederegistrert"
      );
      cy.route("GET", "/api/auth", "fixture:auth");
      cy.route("GET", featureToggles, "fixture:/feature-toggle/nedetid-false");
      cy.route("GET", "/veilarbstepup/status", "fixture:status");
